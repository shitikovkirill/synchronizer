...
from src.planner import Copy, Remove, plan_syncronisation
...
class TestPlaner:
    ...
    def test_get_plann_wirh_same_file(self):
        with patch("builtins.open", mock_open(read_data="1".encode('utf-8'))) as mock_file:
    ...
    def test_get_plann_wirh_diff_file(self):
        with patch("builtins.open", mock_open(read_data="1".encode('utf-8'))) as mock_file:
            file1 = File(Path("test1"))
        with patch("builtins.open", mock_open(read_data="2".encode('utf-8'))) as mock_file:
            file2 = File(Path("test1"))
        sync = set((file1,))
        repl = set((file2,))
        plan = plan_syncronisation(sync, repl)
